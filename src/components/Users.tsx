...
export function Users(props: Props) {
    ...
    function handleSearch(e: FormEvent) {
        ...
        if (value) {
            const users = Array.from({ length: +value }).map((_, idx) => ({id: idx + 1}))
            setCount(users)
        }
    }
    ...
    return (
        <div className="">
            <form className="mb-4 p-2 rounded border border-zinc-200 flex" onSubmit={handleSearch}>
                <input type="number" ref={inputRef} className="px-2 border border-zinc-300 w-full outline-none" />
